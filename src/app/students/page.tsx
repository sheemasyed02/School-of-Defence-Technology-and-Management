    ...
    <>
      ...
      <section className="py-8 sm:py-10 bg-background-muted border-b border-border-light">
        <div className="container-site">
          <StaggerGroup className="grid grid-cols-2 gap-4 sm:gap-6 max-w-md mx-auto">
            {STUDENT_GROUPS.map((g) => (
              <div key={g.label} className="text-center">
                <p className="stat-number text-2xl sm:text-3xl mb-1">{g.students.length}</p>
      ...
      <section className="section-padding bg-background">
        <div className="container-site max-w-4xl">
          <AnimateIn type="fadeUp">
            <p className="text-sm sm:text-base text-foreground-muted leading-relaxed text-center">
          ...
          <div className="container-site relative z-10">
            <AnimateIn type="fadeUp">
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-10">
                <div className={`inline-flex items-center gap-2 sm:gap-3 bg-gradient-to-r ${group.color} rounded-lg sm:rounded-xl px-4 sm:px-5 py-2.5 sm:py-3 shadow-md`}>
                ...
                <div>
                  <h2 className="text-xl sm:text-2xl md:text-3xl font-heading font-bold text-primary">{group.label}</h2>
                  <div className="section-divider mt-2" />
                </div>
              </div>
            </AnimateIn>
            <StaggerGroup className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-5">
              {group.students.map((student) => (
                <div key={student.name} className="group bg-background-paper rounded-xl sm:rounded-2xl shadow-brand border border-border-light flex items-center gap-3 sm:gap-4 p-3 sm:p-4 md:p-5 transition-all duration-300 hover:shadow-brand-lg hover:-translate-y-0.5">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-full overflow-hidden flex-shrink-0 shadow-md group-hover:scale-110 transition-transform duration-300 relative bg-gradient-to-br from-primary to-secondary">
                  ...
                  <p className="text-sm sm:text-base font-semibold text-primary truncate">{student.name}</p>
                </div>
              ))}
            </StaggerGroup>
          </div>
